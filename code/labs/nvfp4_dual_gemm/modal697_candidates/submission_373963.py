  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        const int off_sfb = ((off_n / 128) * rest_k * 512 + iter_k * 2048) / 8;
        tma_1d_gmem2smem_mcast<2>(SFB_smem, SFB_tmap_ptr, off_sfb, mbar_addr, cta_mask, cache_B);

        tma_3d_gmem2smem<2>(A_smem, &A_tmap, 0, off_m, iter_k, mbar_addr, cache_A);
        const int off_sfa = (bid_m * rest_k * 512 + iter_k * 2048) / 8;
        tma_1d_gmem2smem<2>(SFA_smem, &SFA_tmap, off_sfa, mbar_addr, cache_A);
