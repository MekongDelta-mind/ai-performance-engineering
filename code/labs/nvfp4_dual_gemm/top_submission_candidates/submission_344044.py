    ...
    def kernel(
        ...
        if warp_idx < self.mma_warp_id:
            ...
            for subtile_idx in cutlass.range(subtile_cnt):
                ...
                # acc1_vec_silu = epilogue_op(acc1_vec)
                acc1_vec_silu =cute.TensorSSA(
                    silu(acc1_vec, cute.size(acc1_vec.shape), self.tanh),
                    acc1_vec.shape,
                    cutlass.Float32,
                )
                
                tRS_rC.store((acc1_vec_silu * acc2_vec).to(self.c_dtype))
