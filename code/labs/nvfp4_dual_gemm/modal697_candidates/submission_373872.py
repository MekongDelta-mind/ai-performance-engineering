  ...
  if (warp_id == NUM_WARPS - 2) {
    ...
    if (elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        // issue TMA
        // issuing B first is faster than issuing A first
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        if constexpr (ONE_MMA) {
          ...
          tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_FILL   >(c1, a_desc, b1_desc, i_desc, scale_A_tmem, scale_B1_tmem + sfb_offset, iter_k);
          tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_LASTUSE>(c2, a_desc, b2_desc, i_desc, scale_A_tmem, scale_B2_tmem + sfb_offset, iter_k);
        }
        ...
        for (int k = 1; k < BLOCK_K / MMA_K; k++) {
          ...
          if constexpr (ONE_MMA) {
            ...
            tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_FILL   >(c1, a_desc, b1_desc, i_desc, scale_A_tmem, scale_B1_tmem + sfb_offset, 1);
            tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_LASTUSE>(c2, a_desc, b2_desc, i_desc, scale_A_tmem, scale_B2_tmem + sfb_offset, 1);
          }
        }
