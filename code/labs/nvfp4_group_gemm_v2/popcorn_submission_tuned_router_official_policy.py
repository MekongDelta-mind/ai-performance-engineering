...
def _official_semantics_host_defaults() -> tuple[bool, bool, bool, bool]:
    # Matrix evidence (2026-02-28):
    # - graph=0,fuse=1 prefers disabling same-data single-slot/data-fast-cache bookkeeping.
    # - other graph/fuse cells regress when those shortcuts are disabled.
    graph_mode = _env_bool("AISP_NVFP4_GROUP_GEMM_V2_CAPTURE_ITER_GRAPH", False) or _env_bool(
        "AISP_NVFP4_GROUP_GEMM_V2_USE_CUDA_GRAPH", False
    )
    fuse_mode = _env_bool("AISP_NVFP4_GROUP_GEMM_V2_FUSE_INPUTS", True)
    if (not graph_mode) and fuse_mode:
        return True, True, False, False
    return True, True, True, True


(
    _DEFAULT_USE_NATIVE_PTR_UPDATE,
    _DEFAULT_SKIP_PTR_UPDATE_ON_SAME_DATA,
    _DEFAULT_SINGLE_SLOT_FAST,
    _DEFAULT_ENABLE_DATA_FAST_CACHE,
) = _official_semantics_host_defaults()

_USE_NATIVE_PTR_UPDATE = _env_bool("AISP_NVFP4_GROUP_GEMM_NATIVE_PTR_UPDATE", _DEFAULT_USE_NATIVE_PTR_UPDATE)
_SKIP_PTR_UPDATE_ON_SAME_DATA = _env_bool(
    "AISP_NVFP4_GROUP_GEMM_SKIP_PTR_UPDATE_ON_SAME_DATA", _DEFAULT_SKIP_PTR_UPDATE_ON_SAME_DATA
)
_SINGLE_SLOT_FAST = _env_bool("AISP_NVFP4_GROUP_GEMM_SINGLE_SLOT_FAST", _DEFAULT_SINGLE_SLOT_FAST)
_ENABLE_DATA_FAST_CACHE = _env_bool("AISP_NVFP4_GROUP_GEMM_ENABLE_DATA_FAST_CACHE", _DEFAULT_ENABLE_DATA_FAST_CACHE)
