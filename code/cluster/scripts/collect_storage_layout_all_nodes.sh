...
usage() {
  cat <<'EOF' >&2
Usage:
  scripts/collect_storage_layout_all_nodes.sh --hosts <h1,h2,...> [options]
...
SCRIPT_PATH="${ROOT_DIR}/scripts/collect_storage_layout.sh"
...
for idx in "${!HOST_ARR[@]}"; do
  ...
  out_file="${RUN_ID}_${label}_storage.json"
  echo "=== ${label} (${host}) ==="

  # Avoid self-SSH (can be blocked/hang on some clusters); run locally instead.
  ...
  tmp_dir="/tmp/cluster_storage_${RUN_ID}"
  ...
  ssh "${SSH_OPTS[@]}" "${SSH_USER}@${host}" "bash ${tmp_dir}/collect_storage_layout.sh --output ${tmp_dir}/${out_file} --label ${label}" || { echo "Remote storage capture failed for ${host}" >&2; continue; }
