...
def _resolve_capture_iter_graph(default_value: bool) -> bool:
    ...
    """
    override = os.environ.get("AISP_NVFP4_GROUP_GEMM_V2_CAPTURE_ITER_GRAPH")
    if override is None or override.strip() == "":
    ...
    if value in {"0", "false", "no", "off"}:
        return False
    raise ValueError(
        "AISP_NVFP4_GROUP_GEMM_V2_CAPTURE_ITER_GRAPH must be one of "
...
def _resolve_timing_method(default_value: str = "cuda_event") -> str:
    raw = os.environ.get("AISP_NVFP4_GROUP_GEMM_V2_TIMING_METHOD")
    if raw is None or raw.strip() == "":
        return str(default_value)
    value = raw.strip().lower()
    if value not in {"cuda_event", "wall_clock"}:
        raise ValueError(
            "AISP_NVFP4_GROUP_GEMM_V2_TIMING_METHOD must be one of {cuda_event,wall_clock}"
        )
    return value
...
class NVFP4GroupGemmBenchmark(VerificationPayloadMixin, BaseBenchmark):
    ...
    def get_config(self) -> BenchmarkConfig:
        ...
        cross_validate_override = _resolve_optional_bool_env(
            "AISP_NVFP4_GROUP_GEMM_V2_CROSS_VALIDATE_TIMING"
        )
