...
def _load_gemm_v3b() -> object | None:
    ...
    task_mod = _load_module_from_path(module_path.parent / "task.py", "nvfp4_gemm_task_for_gemv_prev")
    utils_mod = _load_module_from_path(module_path.parent / "utils.py", "nvfp4_gemm_utils_for_gemv_prev")
    with _install_module_aliases({"task": task_mod, "utils": utils_mod}):
        ref_mod = _load_module_from_path(
            ...
            "nvfp4_gemm_reference_for_gemv_prev",
        )
    with _install_module_aliases(
        {"task": task_mod, "utils": utils_mod, "reference_submission": ref_mod}
    ):
        mod = _load_module_from_path(module_path, "nvfp4_gemm_opt_for_gemv_prev")
