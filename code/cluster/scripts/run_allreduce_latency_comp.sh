...
usage() {
  cat <<'EOF' >&2
Usage:
  scripts/run_allreduce_latency_comp.sh --hosts <h1,h2,...> [options]

Runs all-reduce latency comparison:
  - one large all-reduce payload
  - many small all-reduces with the same total payload

This is a high-impact diagnostic for communication fragmentation effects.
...
Options:
  ...
  --label <label>        Output label (default: allreduce_latency_comp)
  ...
  --payload-gib <f>      Total payload in GiB (default: 4.0)
  --chunks <n>           Number of small all-reduces (default: 1000)
  ...
  --rdzv-port <port>     Rendezvous port (default: 29502)
EOF
}
...
echo "========================================"
echo "All-reduce latency comparison"
echo "  RUN_ID: ${RUN_ID}"
echo "  LABEL: ${LABEL}"
echo "  Hosts: ${HOSTS}"
echo "  NNODES: ${NNODES}"
echo "  GPUS_PER_NODE: ${GPUS_PER_NODE}"
echo "  Payload: ${PAYLOAD_GIB} GiB, chunks=${CHUNKS}"
...
for idx in "${!HOST_ARR[@]}"; do
  ...
  launch_cmd=(
    ...
    --max_restarts=0
    "${REMOTE_ROOT}/scripts/allreduce_latency_comp.py"
...
if [[ "$fail" -ne 0 ]]; then
  echo "ERROR: all-reduce latency comparison failed; check results/raw logs." >&2
  exit 1
fi
...
echo ""
echo "All-reduce latency comparison complete."
echo "Output: ${OUT_STRUCT_REL}"
