...
template <int CTA_GROUP = 1>
__device__ inline
void tma_1d_gmem2smem_multicast(int dst, const void *tmap_ptr, int x, int mbar_addr, int16_t cta_mask, uint64_t cache_policy) {
    ...
    if (elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        if constexpr (ONE_MMA) {
          tma_3d_gmem2smem<2>(B_smem, B_tmap_ptr, 0, off_n, iter_k, mbar_addr, cache_B);
        } else {
          tma_3d_gmem2smem<2>(B_smem,              &B1_tmap, 0, b_off_n, iter_k, mbar_addr, cache_B);
          tma_3d_gmem2smem<2>(B_smem + B_size / 2, &B2_tmap, 0, b_off_n, iter_k, mbar_addr, cache_B);
        }
        ...
        tma_1d_gmem2smem_multicast<2>(SFB_smem, SFB_tmap_ptr, off_sfb / 8, mbar_addr, cta_mask, cache_B);
    ...
    if (cta_rank == 0 && elect_sync()) {
      ...
      for (int iter_k = 0; iter_k < num_iters; iter_k++) {
        ...
        for (int k = 0; k < BLOCK_K / MMA_K; k++) {
          ...
          tcgen05_cp_nvfp4<2>(scale_B_tmem, sfb1_desc);
          tcgen05_cp_nvfp4<2>(scale_B_tmem + 4, sfb2_desc);

          if constexpr (ONE_MMA) {
            tcgen05_mma_nvfp4<2>(0, a_desc, b_desc, i_desc, scale_A_tmem, scale_B_tmem, enable_input_d);
          } else {
            tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_FILL   >(      0, a_desc, b_desc,                 i_desc, scale_A_tmem, scale_B_tmem + sfb_offset,     enable_input_d);
            tcgen05_mma_nvfp4<2, COLLECTOR_USAGE::A_LASTUSE>(BLOCK_N, a_desc, b_desc + (B_size >> 5), i_desc, scale_A_tmem, scale_B_tmem + sfb_offset + 4, enable_input_d);
          }
