...
## How To Read Tier-1
- `summary.json` is the canonical machine-readable result for the run. It carries per-target baseline time, best speedup, and artifact references.
- `regression_summary.md` is the human-facing "what changed since the last canonical run?" document.
- `trend_snapshot.json` is the compact time-series summary for dashboards and release notes.
