...
salloc --nodes=4 --ntasks-per-node=1 --cpus-per-task=48 --gres=gpu:8 bash

srun --gres=gpu:8 --nodes=4 --tasks-per-node=1 python3 -u -m torch.distributed.run --nproc_per_node=8 --nnodes 4 --rdzv_endpoint $(scontrol show hostnames $SLURM_JOB_NODELIST | head -n 1):6000 --rdzv_backend c10d all_reduce_bench.py
...
fmt_bytes = lambda v: str(v >> ((max(v.bit_length() - 1, 0) // 10) * 10)) + ["", "K", "M", "G", "T", "P", "E"][
    max(v.bit_length() - 1, 0) // 10
] + "iB"
...
def run(local_rank):
    ...
    def finish():
        ...
        print("Legend: 1KiB = 2**10Bytes, 1MiB = 2**20Bytes, 1GiB = 2**30Bytes")
        print("        1GBps = 10**9Bytes per second (networking bw spec convention)")
        print(f"Elapsed time: {time_str}")

        # Save benchmark results to JSON file for integration with summary
        benchmark_results = {
            ...
            "test_info": {
                ...
                "warmups": WARMUPS,
                "trials": TRIALS,
                "elapsed_time_seconds": time_delta,
                "device_info": get_device_info(),
            },
            "metrics": {
                "busbw": {
                    "values_gbps": [conv_to_GBps(busbw[size]) for size in busbw.keys()],
                    "avg_gbps": sum(conv_to_GBps(busbw[size]) for size in busbw.keys()) / len(busbw),
                    "max_gbps": max(conv_to_GBps(busbw[size]) for size in busbw.keys()),
                    "detailed": {fmt_bytes(size): conv_to_GBps(busbw[size]) for size in busbw.keys()},
                },
                "algbw": {
                    "values_gbps": [conv_to_GBps(algbw[size]) for size in algbw.keys()],
                    "avg_gbps": sum(conv_to_GBps(algbw[size]) for size in algbw.keys()) / len(algbw),
                    "max_gbps": max(conv_to_GBps(algbw[size]) for size in algbw.keys()),
                    "detailed": {fmt_bytes(size): conv_to_GBps(algbw[size]) for size in algbw.keys()},
                },
            },
        }
        ...
        results_file = logs_dir / f"allreduce_benchmark_results_{jobid}_{hostname}_{datetime.datetime.now().strftime('%Y%m%d_%H%M%S')}.json"

        with open(results_file, "w") as f:
            json.dump(benchmark_results, f, indent=2)

        print(f"\n*** Benchmark results saved to: {results_file}")
        print(f"    Average busbw: {benchmark_results['metrics']['busbw']['avg_gbps']:.2f} GBps")
        print(f"    Maximum busbw: {benchmark_results['metrics']['busbw']['max_gbps']:.2f} GBps")
        print(f"    Average algbw: {benchmark_results['metrics']['algbw']['avg_gbps']:.2f} GBps")
        print(f"    Maximum algbw: {benchmark_results['metrics']['algbw']['max_gbps']:.2f} GBps")
