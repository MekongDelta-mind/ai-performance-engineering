...
usage() {
  cat <<'EOF' >&2
Usage:
  scripts/run_discovery_all_nodes.sh --hosts <h1,h2,...> [options]
...
SCRIPT_PATH="${ROOT_DIR}/scripts/collect_system_info.sh"
...
for idx in "${!HOST_ARR[@]}"; do
  ...
  out_file="${RUN_ID}_${label}_meta.json"
  echo "=== ${label} (${host}) ==="
  ...
  tmp_dir="/tmp/cluster_discovery_${RUN_ID}"
  ...
  ssh "${SSH_OPTS[@]}" "${SSH_USER}@${host}" "bash ${tmp_dir}/collect_system_info.sh --output ${tmp_dir}/${out_file} --label ${label}" || { echo "Remote discovery failed for ${host}" >&2; continue; }
